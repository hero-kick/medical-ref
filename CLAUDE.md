...
- `#search` の `input` イベント — `data-search` 属性（title + shortLabel + keywords + 本文テキスト + noteTemplate の結合）でスペース区切り AND 検索。カテゴリチップ（`#cat-chips`、ITEMS の `category` から動的生成）との複合絞り込み
- ピン留め（`.pin-btn`）とテーマ切替（`#theme-toggle`）は localStorage（`medref-pins` / `medref-theme`）に保存。file:// 等で localStorage 不可でも動くよう `storeGet`/`storeSet` で try/catch 済み
- 記載メモのコピー（`.copy-btn`）は `navigator.clipboard` 不可時に `document.execCommand("copy")` へフォールバック
- **注意**: `build_standalone.py` の正規表現の都合上、`<script>` 内に `(function () {` パターンを PWA 用 IIFE 以外で書かないこと（アロー関数か名前付き関数を使う）
...
| `title` | string | 表示タイトル |
| `category` | string | 絞り込みチップ用カテゴリ（既存: 制度・区分 / 薬剤 / 書類 / 身体所見）。チップは ITEMS から動的生成 |
